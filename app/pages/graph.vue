  ...
  <v-container fluid class="pa-4 pa-sm-6">
    <div class="d-flex align-center mb-4">
      ...
      <v-chip size="small" variant="tonal" color="primary">
        {{ nodeCount.dates }} dates, {{ nodeCount.pages }} pages, {{ nodeCount.people }} people, {{ nodeCount.locations }} locations
      </v-chip>
      <v-btn
        icon="mdi-refresh"
        variant="text"
...
const nodeCount = computed(() => {
  ...
  const people = nodes.filter(n => n.data.type === 'person').length
  const locations = nodes.filter(n => n.data.type === 'location').length
  return { total: nodes.length, dates, pages, people, locations }
})
